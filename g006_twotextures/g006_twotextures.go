...
func init1() {
  ...
  win, err = sdl.CreateWindow("g006 Two Textures", 200, 100, sW, sH, sdl.WINDOW_OPENGL)
  if err != nil { panic(err) }
  ...
  /*   X     Y  Z    Color RGB   tX tY - texture coordinates*/
    -0.5, -0.8, 0,   0, 0.5, 0,   0, 1, /*tY flipped because Y points up*/
     0.5, -0.8, 0,   0, 0.5, 0,   1, 1,
     0.5,  0.8, 0,   0,   0, 1,   1, 0,
    -0.5,  0.8, 0,   0, 0.5, 1,   0, 0,
  }
...
func draw() {
  ...
  t = sdl.GetTicks()
  globalColor = float32((math.Sin(float64(t) / 700) + 1) / 2)
